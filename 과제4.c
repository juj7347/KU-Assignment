...
int rbt_count(RBTPtr self, NodePtr tree){
	...
	if (tree->left != self->NIL)
		...
		return rbt_count(self, tree->left) + rbt_count(self, tree->right) ;
}
...
void total_counter(RBTPtr self, NodePtr root) {
	if (root != self->NIL) {
		total_counter(self, root->left);
		total++;
		total_counter(self, root->right);
	}
}
...
int main(){
	...
	FILE *fp;
	int num;
	fp = fopen("C:\\Users\\정윤재\\Downloads\\rbtest\\test01.txt", "r");
	while (!feof(fp)){
		fscanf(fp, "%d", &num);
		if (num > 0){
	...
	fclose(fp);
	rbt_bh(rbt, rbt->root);
	total_counter(rbt, rbt->root);
	//rbt_print(rbt, rbt->root, 0); 트리의 모양을 출력해주는 함수
